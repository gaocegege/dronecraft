...
const (
	baseURL = "https://get.docker.com/builds/Linux/x86_64/docker-"
)
...
func main() {
	...
	logrus.Println("looking for docker binary named:", dockerBinaryName, "in ", filename)
	...
	if _, err := os.Stat(filename); os.IsNotExist(err) {
		...
		if versionComp >= 0 {
			getClient(out, baseURL+dockerDaemonVersion+".tgz", extractClient)
		} else {
			getClient(out, baseURL+dockerDaemonVersion, copyClient)
		}
